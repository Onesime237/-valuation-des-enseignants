...
<div class="main">
    ...
    <div class="form-card">
        <h2 style="font-size:1.2rem;font-weight:700;margin-bottom:1.5rem">Edit Course</h2>
        ...
        <form action="/admin/courses/edit/<?= esc($course['id']) ?>" method="POST">
            <?= csrf_field() ?>

            <div class="mb-3">
                <label class="form-label">Course Name</label>
                <input type="text" name="name" class="form-control" value="<?= esc(old('name', $course['name'])) ?>" required>
            </div>

            <div class="mb-4">
                <label class="form-label">Description <span style="color:#475569">(optional)</span></label>
                <textarea name="description" class="form-control" rows="3"><?= esc(old('description', $course['description'])) ?></textarea>
            </div>

            <button type="submit" class="btn w-100" style="background:#6366f1;color:#fff;border-radius:8px;font-weight:600">Update Course</button>
        </form>
    </div>
</div>
</body>
