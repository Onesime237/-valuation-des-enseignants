...
<head>
    ...
    <title>Create Question — Admin</title>
...
<div class="main">
    ...
    <div class="form-card">
        <h2 style="font-size:1.2rem;font-weight:700;margin-bottom:1.5rem">New Question</h2>
        ...
        <form action="/admin/questions/create" method="POST">
            ...
            <div class="mb-3">
                ...
                <textarea name="text" class="form-control" rows="3" placeholder="e.g. How would you rate the teacher's clarity?" required><?= esc(old('text')) ?></textarea>
            </div>

            <div class="mb-3">
                <label class="form-label">Question Type</label>
                <select name="type" class="form-select" required>
                    <option value="scored" <?= old('type') === 'scored' ? 'selected' : '' ?>>Scored (1-5 rating)</option>
                    <option value="open" <?= old('type') === 'open' ? 'selected' : '' ?>>Open Text (free response)</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="category_id" class="form-select">
                    ...
                    <?php foreach ($categories as $id => $name): ?>
                        <option value="<?= esc($id) ?>" <?= old('category_id') == $id ? 'selected' : '' ?>><?= esc($name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>

            <button type="submit" class="btn w-100" style="background:#6366f1;color:#fff;border-radius:8px;font-weight:600">Create Question</button>
        </form>
    </div>
</div>
</body>
